...
crec(ContextId, CallerId, Pid, Call, Result, EndTime, EndReds) ->
    ...
    #crec{id = Id,
          ...
          args = Call#call.args,
          result = Result,
          return = normal,               %TODO Don't use normal as default value
...
find_crecs(CallerId) ->
    MS = ets:fun2ms(fun(#rec{data=#crec{caller_id=Id}}=A)
                          when Id == CallerId ->
                            A
                    end),
...
find_crecs(Module, Func) ->
    MatchSpec = ets:fun2ms(
                  fun(#rec{data=#crec{module=M1, func=F1}}=A)
                        when M1 == Module, F1 == Func ->
                          A
                  end),
...
track_summary(ContextId, Pid, Call, Arity, EndTime, EndReds) ->
    ...
    SRec1 =
        case find(Id) of
            #srec{} = SRec->
                SRec#srec{reds = SRec#srec.reds + Reds,
